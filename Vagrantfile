...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    # Define the vm
    vm_name = "moat"
    config.vm.define :moat do |moat|
        moat.vm.network :private_network, ip: "172.16.16.16"
        moat.vm.hostname = vm_name

        moat.vm.synced_folder "salt/roots/", "/srv/"
        moat.vm.synced_folder ".", "/src/"

        moat.vm.network :forwarded_port, guest: 80, host: 80, auto_correct: true

        moat.vm.provider "virtualbox" do |v|
            v.name = vm_name
        ...
        moat.vm.provision :salt do |config|
